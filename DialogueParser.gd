...
func init_dialogue(target):
	...
	if currDialogue[1] != null:
		...
		if currDialogue[1].has("linkPath"):
			isChoice = true
			isChoiceDialogue = true
	#panelNode.get_node("RichTextLabel").set_text(currDialogue[0])
	panelNode.print_dialogue(currDialogue[0])

func _ready():
	set_process_input(true)
	
	load_file_as_JSON("res://game/dialogue/storyTest.json", myStory)
	load_file_as_JSON("res://game/dialogue/events.json", events)
	load_file_as_JSON("res://game/dialogue/choices.json", choices)
	var directory = Directory.new();
	var doFileExists = directory.file_exists("res://game/dialogue/choices.json")
	print(doFileExists)
	print(choices)
	panelNode = get_node("../Player/Camera2D/CanvasLayer/Dialogue Box")
